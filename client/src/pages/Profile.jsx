                ...
                <div className="space-y-6">
                  {orders.map((order) => (
                    <div key={order._id} className="bg-white dark:bg-black border border-gray-300 dark:border-gray-700 rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow">
                      {/* Order Header */}
                      <div className="bg-gray-50 dark:bg-gray-900 px-4 sm:px-6 py-4 border-b border-gray-300 dark:border-gray-700">
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
                          ...
                          <div className="flex gap-2">
                            {order.isDelivered ? (
                              <span className="px-3 sm:px-4 py-1.5 rounded-full text-xs font-semibold bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200">
                                ✓ Delivered & Paid
                              </span>
                            ) : (
                              <span
                      ...
                      <div className="p-4 sm:p-6">
                        <div className="space-y-4">
                          {order.orderItems.map((item, index) => (
                            <div key={index} className="flex flex-col sm:flex-row gap-3 sm:gap-4 items-start sm:items-center p-3 sm:p-4 bg-gray-50 dark:bg-gray-900 rounded-xl">
                              <Link to={`/product/${item.product}`} className="flex-shrink-0">
                                <img
                                  src={item.image}
                                  alt={item.name}
                                  className="w-16 h-16 sm:w-20 sm:h-20 object-cover rounded-lg border border-gray-200 dark:border-gray-700 hover:opacity-75 transition-opacity cursor-pointer"
                                />
                              </Link>
                              <div className="flex-1 min-w-0">
                                <Link 
                                  to={`/product/${item.product}`}
                                  className="font-semibold mb-1 hover:underline block text-sm sm:text-base"
                                >
                                  {item.name}
                                </Link>
                                <div className="flex flex-wrap items-center gap-2 sm:gap-3 text-xs sm:text-sm text-gray-600 dark:text-gray-400">
                                  ...
                                  <span className="hidden sm:inline">₹{formatPrice(item.price)} each</span>
                                </div>
                              </div>
                              <p className="font-bold text-base sm:text-lg self-end sm:self-auto">₹{formatPrice(item.price * item.qty)}</p>
                            </div>
                          ))}
                        </div>

                        {/* Order Footer */}
                        <div className="mt-6 pt-6 border-t border-gray-300 dark:border-gray-700 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                          <Link
                            to={`/order/${order._id}`}
                          ...
                          <div className="text-left sm:text-right w-full sm:w-auto">
                            ...
                            <p className="text-xl sm:text-2xl font-bold">₹{formatPrice(order.totalPrice)}</p>
                          </div>
                        </div>
                      </div>
                    </div>
